...
function ip() {
  const discordUrl = DISCORD || 'https://discord.gg/972PPSN7Fy';
  if (!config.ouvert) {
...
<section class="section"><div class="enveloppe">
  ...
  <div class="etapes">
    ...
    <div class="panneau etape"><span class="num">2</span><h3>${config.ouvert ? 'Ajoute le serveur' : 'Rejoins le Discord'}</h3><p>${config.ouvert ? `Multijoueur, Ajouter un serveur, adresse <b>${esc(config.adresseJeu)}</b>. Accepte le pack du serveur.` : `L'adresse IP officielle sera dévoilée sur notre Discord avant le grand lancement : <a href="${esc(DISCORD || 'https://discord.gg/972PPSN7Fy')}" target="_blank" rel="noopener" class="lien"><b>Rejoindre la communauté</b></a>.`}</p></div>
  ...
  <div>
    <div class="panneau">
      ...
      <form method="POST" action="/contact">
        ...
        <div class="champ">
          <label for="email">Ton adresse email <span style="color:var(--couleur-rouge)">*</span></label>
          <input type="email" id="email" name="email" placeholder="ton.email@exemple.com" required maxlength="100">
        </div>
        <div class="champ">
          <label for="sujet">Sujet de ta demande <span style="color:var(--couleur-rouge)">*</span></label>
        ...
        <div class="champ">
          <label for="message">Message <span style="color:var(--couleur-rouge)">*</span></label>
          <textarea id="message" name="message" rows="5" placeholder="Explique-nous ta demande en détail..." required maxlength="2000"></textarea>
        </div>
        <button type="submit" class="btn btn-plein">Envoyer mon message</button>
      </form>
    </div>
  </div>
</div></section>`;
